...
uploaded_files = None
if is_admin:
    st.sidebar.markdown("### ⚙️ Painel da proprietária")
...
with tabs[1]:
    ...
    if not df_atend.empty:
        ...
        with k_at3:
            st.markdown(f"""<div class="kpi-card" style="border-left: 4px solid {BRAND_PEACH_DEEP};"><div class="kpi-title">Modalidade predominante</div><div class="kpi-value" style="font-size:1.25rem;">{top_mod}</div><div class="kpi-subtitle">{top_mod_count} contatos ({(top_mod_count/total_at*100):.1f}%)</div></div>""", unsafe_allow_html=True)
        with k_at4:
            st.markdown(f"""<div class="kpi-card" style="border-left: 4px solid #8B7BB8;"><div class="kpi-title">📞 Telefone (mensagem)</div><div class="kpi-value">{total_telefone}</div><div class="kpi-subtitle">{(total_telefone/total_at*100 if total_at else 0):.1f}% • WhatsApp e ligações</div></div>""", unsafe_allow_html=True)
        with k_at5:
            st.markdown(f"""<div class="kpi-card" style="border-left: 4px solid {BRAND_GREEN};"><div class="kpi-title">🎥 Atendimentos por vídeo</div><div class="kpi-value">{total_video}</div><div class="kpi-subtitle">{(total_video/total_at*100 if total_at else 0):.1f}% • Sessões por vídeo</div></div>""", unsafe_allow_html=True)

        st.markdown(f"""
        <div style="background-color: #F8F6FD; border-left: 4px solid {BRAND_PURPLE}; padding: 0.85rem 1.2rem; border-radius: 10px; margin-top: 0.8rem; margin-bottom: 1.2rem;">
            <p style="margin: 0; font-size: 0.88rem; color: {BRAND_PURPLE_DARK}; line-height: 1.5;">
                💡 <b>Especificação dos atendimentos:</b> Os atendimentos de <b>telefone (mensagem)</b> correspondem a contatos contínuos por mensagens (WhatsApp) e ligações da equipe para suporte e acolhimento. Já os atendimentos de <b>vídeo</b> representam as sessões estruturadas por videochamada. Em agosto/2026, o volume de <b>405 atendimentos</b> reflete a intensificação desses contatos por mensagens (320) somado a 62 videochamadas e 23 articulações de rede.
            </p>
        </div>
        """, unsafe_allow_html=True)

        # Gráfico 1: Evolução Mensal dos Atendimentos com Especificação por Modalidade
        meses_ordem_keys = ['2026-03', '2026-04', '2026-05', '2026-06', '2026-07', '2026-08']
        meses_map_pt = {
            '2026-03': 'Mar/2026', '2026-04': 'Abr/2026', '2026-05': 'Mai/2026',
            '2026-06': 'Jun/2026', '2026-07': 'Jul/2026', '2026-08': 'Ago/2026'
        }
        df_atend_plot = df_atend.copy()
        df_atend_plot['mes_rotulo'] = df_atend_plot['mes_ano'].map(meses_map_pt).fillna(df_atend_plot['mes_ano'])
        ordem_meses_visiveis = [meses_map_pt[k] for k in meses_ordem_keys if k in df_atend['mes_ano'].unique()]

        def simplificar_modalidade(m):
            if m in ['Telefone (mensagem)', 'Atendimento Telefônico']:
                return 'Telefone (mensagem)'
            elif m in ['Atendimento por vídeo', 'Atendimento por Vídeo']:
                return 'Atendimento por vídeo'
            else:
                return 'Articulação de rede / outros'

        df_atend_plot['modalidade_destaque'] = df_atend_plot['categoria_modalidade'].apply(simplificar_modalidade)
        df_at_modalidade = df_atend_plot.groupby(['mes_rotulo', 'modalidade_destaque']).size().reset_index(name='Atendimentos')

        col_ctrl1, col_ctrl2 = st.columns([3, 1])
        with col_ctrl1:
            st.markdown("#### 📈 Volume mensal de atendimentos por modalidade (2026)")
        with col_ctrl2:
        ...
        bmode = 'group' if modo_grafico == "Lado a lado" else 'stack'
        cores_modalidades = {
            'Telefone (mensagem)': BRAND_PURPLE,
            'Atendimento por vídeo': BRAND_GREEN,
            'Articulação de rede / outros': BRAND_PEACH_DEEP
        }
