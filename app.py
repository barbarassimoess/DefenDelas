...
with tabs[0]:
    ...
    with col4:
        st.markdown(f"""<div class="kpi-card" style="border-left: 4px solid {BRAND_PURPLE_DARK};"><div class="kpi-title">📍 Cidades alcançadas</div><div class="kpi-value">{cidades_unicas}</div><div class="kpi-subtitle">Municípios de SC atendidos</div></div>""", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    if not df_atend.empty:
        ...
        with k_at4:
            st.markdown(f"""<div class="kpi-card" style="border-left: 4px solid {BRAND_PURPLE_DARK};"><div class="kpi-title">📞 Telefone (mensagem)</div><div class="kpi-value">{total_telefone}</div><div class="kpi-subtitle">{(total_telefone/total_at*100 if total_at else 0):.1f}% • WhatsApp e ligações</div></div>""", unsafe_allow_html=True)
        with k_at5:
        ...
        <div style="background-color: #F8F6FD; border-left: 4px solid {BRAND_PURPLE}; padding: 0.85rem 1.2rem; border-radius: 10px; margin-top: 0.8rem; margin-bottom: 1.2rem;">
            <p style="margin: 0; font-size: 0.88rem; color: {BRAND_PURPLE_DARK}; line-height: 1.5;">
                💡 <b>Especificação dos atendimentos:</b> Os atendimentos de <b>telefone (mensagem)</b> correspondem a contatos contínuos por mensagens (WhatsApp) e ligações da equipe para suporte e acolhimento. Já os atendimentos de <b>vídeo</b> representam as sessões estruturadas por videochamada.
            </p>
        </div>
        """, unsafe_allow_html=True)
...
with tabs[5]:
    ...
    if not df.empty:
        ...
        with c_va1:
            ...
            apply_chart_theme(fig_agr, left_margin=200)
